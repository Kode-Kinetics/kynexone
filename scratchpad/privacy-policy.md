...
|---|---|---|---|
| Model is **deepseek-v4-pro:cloud** on Ollama Cloud | `AI_PROVIDER=ollama`, `OLLAMA_BASE_URL=https://ollama.com`, `AI_MODEL` | `render.yaml:170-179` (provider + URL in the blueprint; `AI_MODEL` is `sync: false`). **CONFIRMED 2026-09-23 against the Render dashboard: `AI_MODEL=deepseek-v4-pro:cloud`.** The page said `gpt-oss:120b` from publication until then — exactly the drift this row was written to catch. `docker-compose.yml:39` still defaults to `gpt-oss:120b-cloud` for local dev and does not govern production. Re-confirm whenever `AI_MODEL` changes; the privacy page names the model explicitly. | Render dashboard `AI_MODEL` |
