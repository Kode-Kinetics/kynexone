...
public class PlatformEmailSettingsTests : PlatformTestBase
{
    ...
    public void SecureOptionFor_MatchesTheSubmissionPortsSemantics(int port, bool useTls, SecureSocketOptions expected)
        => SmtpEmailService.SecureOptionFor(port, useTls).Should().Be(expected);
