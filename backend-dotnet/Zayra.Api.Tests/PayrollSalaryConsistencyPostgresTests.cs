...
public sealed class PayrollSalaryConsistencyPostgresTests
{
    ...
    public async Task IntelliFlowSeed_LockedRunCoverageYtdAndGosiAgree()
    {
        ...
        var gosi = await new GosiReadinessReportService(db, new Zayra.Api.Infrastructure.CountryPack.StatutoryRuleReader(db)).BuildAsync(tenant.Id, CancellationToken.None);
