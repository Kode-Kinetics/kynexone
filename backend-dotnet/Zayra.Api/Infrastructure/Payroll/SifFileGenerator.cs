...
///
/// SOMEBODY HAS NOW DONE THE CHECK THIS COMMENT ASKED FOR — AND IT FAILS.
///   The paragraph above was true and correct, and is deliberately left exactly as it
///   was. The MHRSD specification it points at was retrieved on 2026-09-20 and the
///   comparison run. The result is worse than "unconfirmed widths": this layout is not
///   a Saudi WPS file and cannot be made into one.
///
///   The published spec requires a TAB-delimited UTF-8 .txt file of bracketed
///   SWIFT-style FIELD tags carrying the per-employee wage BREAKDOWN, with a COMMA
///   decimal separator — and the file an employer uploads is produced and digitally
///   signed BY THE BANK, with the establishment→bank transfer explicitly out of the
///   spec's scope. "EDI_DC40" and "E1EDL20" are SAP IDoc structures (control record
///   and logistics delivery header) and appear in no wage-protection specification
///   anywhere, Saudi or UAE.
///
///   <see cref="WpsConformance"/> holds the full finding with its citation, and rides
///   on the validation response, the generate response and the X-Wps-Conformance
///   header of the download, so the claim a CUSTOMER sees matches this comment.
///
///   Nothing about the generated BYTES changed: the file is content-addressed by
///   SHA-256 and its determinism is pinned by tests. This artefact remains useful as a
///   payroll instruction to hand a bank; it is simply not a WPS submission, and the
///   product no longer says it is. Closing it for real is procurement — a bank's
///   payroll-file format, or Mudad PMS partner access. See WpsConformance.Remediation.
/// </summary>
