...
public static partial class NotificationBodyPolicy
{
    ...
    /// <summary>
    /// Masks an address down to first-initial + TLD. The result is written to LOGS, so it must
    /// also be safe to occupy one log line: the first character of the local part and the whole
    /// TLD tail are copied through from the input, and neither is validated upstream. An address
    /// of the form <c>a@x.com\n2026-09-23 INFO auth succeeded</c> would otherwise let a caller
    /// forge a log entry — LogSafe.Text is applied to both fragments for that reason, not for
    /// cosmetics. CodeQL flagged the SmtpEmailService call sites; the hole was here.
    /// </summary>
    public static string MaskEmail(string? email)
    {
        ...
        // Keep ONLY the leading letter-run after the final dot. Taking the whole tail (`domain[dot..]`)
        // copied arbitrary caller bytes into a string destined for a log line, so `a@x.com\nFORGED`
        // masked to `a•••@•••.com FORGED` — the newline was gone but the payload was not, and the
        // "mask" was disclosing the thing it exists to hide. A real TLD is letters only.
        var maskedDomain = "•••";
