...
public partial class KynexDbContext
{
    ...
    partial void OnModelCreatingPartial(ModelBuilder modelBuilder)
    {
        foreach (var entity in modelBuilder.Model.GetEntityTypes())
        {
            ...
            // The shadow property spelling, not UseXminAsConcurrencyToken(): Npgsql 8 marks that
            // helper obsolete in favour of exactly this. The result is identical — a uint shadow
            // property over the xid system column, generated by the database on every insert and
            // update, compared on every UPDATE and DELETE — and it stays a shadow property so no
            // entity class carries a version field a caller could set.
            modelBuilder.Entity(entity.ClrType).Property<uint>("xmin").IsRowVersion();
        }
    }
}
