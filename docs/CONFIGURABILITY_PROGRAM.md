...
| OD-4 | Country scope: which of the 16 rule-only countries get executable packs, and is the no-pack behavior hard-block or acknowledged-warning? | Program input still open per go-live backlog. Recommendation: hard-block by default, per-tenant acknowledged-warning override for reference-only use | Wave 1 |
